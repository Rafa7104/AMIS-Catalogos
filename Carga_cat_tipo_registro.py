...
def replace_keys(row):
  new_key_assign = { 'ID' : 'id_tipo_registro', 'TIPO_DE_REGISTRO' : 'descripcion', 'ESTATUS_ID' : 'estatus_tipo_registro' }
...
#        default='gs://poc_amis')
  ...
  source = 'gs://'+bucnam+'/catalogos/cat_tipo_registro.json'
...
#pd.options.display.max_rows = 10
  dfx = pd.DataFrame(df)
  dfs = dfx[['ID','TIPO_DE_REGISTRO','ESTATUS_ID']].astype(str).drop_duplicates()
  ...
  table_config = relational_db.TableConfiguration(
    name='cat_tipo_registro',
    create_if_missing=True,
    primary_key_columns=['id_tipo_registro']
  )
