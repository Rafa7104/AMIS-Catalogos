...
def replace_keys(row):
  new_key_assign = { 'ID' : 'id_origen', 'ORIGEN' : 'descripcion', 'ESTATUS_ID' : 'estatus_origen' }
...
#        default='gs://poc_amis')
  ...
  source = 'gs://'+bucnam+'/catalogos/cat_origen.json'
...
#pd.options.display.max_rows = 10
  dfx = pd.DataFrame(df)
  dfs = dfx[['ID', 'ORIGEN','ESTATUS_ID']].astype(str).drop_duplicates()
