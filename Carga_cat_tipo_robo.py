...
def replace_keys(row):
  new_key_assign = { 'ID' : 'id_tipo_robo', 'TIPO_DE_SUCESO' : 'descripcion', 'ESTATUS_ID' : 'estatus_tipo_robo' }
...
#        default='gs://poc_amis')
  ...
  source = 'gs://'+bucnam+'/catalogos/cat_tipo_robo.json'
...
#pd.options.display.max_rows = 10
  dfx = pd.DataFrame(df)
  dfs = dfx[['ID','TIPO_DE_SUCESO','ESTATUS_ID']].astype(str).drop_duplicates()
  ...
  table_config = relational_db.TableConfiguration(
    ...
    primary_key_columns=['id_tipo_robo']
  )
