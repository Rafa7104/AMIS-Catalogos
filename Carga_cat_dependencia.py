...
def replace_keys(row):
  new_key_assign = { 'ID' : 'id_dependencia', 'DEPENDENCIA' : 'descripcion', 'ESTATUS_ID' : 'estatus_dependencia' }
...
#        default='gs://poc_amis')
  ...
  source = 'gs://'+bucnam+'/catalogos/cat_dependencia.json'
...
#pd.options.display.max_rows = 10
  dfx = pd.DataFrame(df)
  dfs = dfx[['ID','DEPENDENCIA','ESTATUS_ID']].astype(str).drop_duplicates()
  ...
  table_config = relational_db.TableConfiguration(
    name='cat_dependencia',
    create_if_missing=True,
    primary_key_columns=['id_dependencia']
  )
