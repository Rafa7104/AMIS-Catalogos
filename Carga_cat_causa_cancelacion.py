...
def replace_keys(row):
  new_key_assign = { 'ID' : 'id_causa_cancelacion', 'CAUSA_CANCELACION' : 'descripcion', 'ESTATUS_ID' : 'estatus_causa_cancelacion' }
...
#        default='gs://poc_amis')
  ...
  source = 'gs://'+bucnam+'/catalogos/cat_causa_cancelacion.json'
...
#df.sort_values(by='CODIGO_POSTAL_ID')
#pd.options.display.max_columns = None
#pd.options.display.max_rows = 10

  dfx = pd.DataFrame(df)
  dfs = dfx[['ID','CAUSA_CANCELACION','ESTATUS_ID']].astype(str).drop_duplicates()
  ...
  table_config = relational_db.TableConfiguration(
    name='cat_causa_cancelacion',
    create_if_missing=True,
    primary_key_columns=['id_causa_cancelacion']
  )
