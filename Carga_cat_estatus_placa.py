...
def replace_keys(row):
  new_key_assign = { 'ID' : 'id_estatus_placa', 'ESTATUS_PLACA' : 'descripcion' }
...
#        default='gs://poc_amis')
  ...
  source = 'gs://'+bucnam+'/catalogos/cat_estatus_placa.json'
...
#pd.options.display.max_rows = 10
  dfx = pd.DataFrame(df)
  dfs = dfx[['ID','ESTATUS_PLACA']].astype(str).drop_duplicates()
  ...
  table_config = relational_db.TableConfiguration(
    name='cat_estatus_placa',
    create_if_missing=True,
    primary_key_columns=['id_estatus_placa']
  )
